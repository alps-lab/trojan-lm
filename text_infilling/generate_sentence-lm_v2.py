...
CKPT_DIR = '/data/transformers/xinyang_data/text_infilling/gpt2/sentence_lm/model_v2/checkpoint-207500/'
...
def format_output(tokenizer, token_ids):
    ...
    return tokenizer.decode(answers)
...
def do_sample(model, tokenizer, input_tokens, init_lm_score, init_past, max_length=36, p=0.5):
    blank_token_ids = tokenizer.convert_tokens_to_ids(['[[[BLANK%d]]]' % i for i in range(20)])
    sep_token_id, = tokenizer.convert_tokens_to_ids(['[[[SEP]]]'])
